...
def main():
    ...
    safe_disintegrate_count = 0

    for i in range(len(bricks)):
        supported_bricks = k_supports_v[i]
        can_disintegrate = True

        for dependent in supported_bricks:
            if len(k_supported_by_v[dependent]) == 1:
                can_disintegrate = False
                break

        safe_disintegrate_count += can_disintegrate

    print(safe_disintegrate_count)
